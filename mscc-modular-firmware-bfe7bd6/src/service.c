// Copyright (c) 2018 Microsemi Corporation "Microsemi".
...
Service_t *service_load(char profile[64])
{
    ...
    while ((in_file = readdir(FD))) {
        ...
        strncat(filepath, in_file->d_name, sizeof(filepath));
...
void service_start(Service_t *s, int *forked)
{
    int i, fd_max;
    Attr_t *elt;
    int num_ser_env, j;
                                 ...
                                 enum OnError     action_taken)
{
    ...
    printf("\nService \"%s\" of type = \"%s\" with PID = %d ",
           service_name ? service_name : "<Unnamed>",
...
void service_update_wait_status(Service_t *head, pid_t pid, int status)
{
    ...
    if (action_taken == REBOOT) {
        ...
        // Sync any unsaved files and write-buffers in UBIFS
        sync();

        // Unmount the UBIFS mount point. Use (the) force as we are rebooting
