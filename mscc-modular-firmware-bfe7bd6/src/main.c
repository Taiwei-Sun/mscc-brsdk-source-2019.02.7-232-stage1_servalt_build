...
int cmdline_read(CmdLine *cl) {
    ...
    if ((p = strstr(buf, "switch=nomount"))) {
        info("Disabling mounting of ubi0:switch\n");
...
char *get_stage2_device(int *mtd_idx) {
    ...
    if (mtd_device_by_name("rootfs_data", buf, 128, mtd_idx)) {
        return buf;
    } else {
        return NULL;
    }
...
int process_stage2_fd(int fd, int fd_loop, size_t off, size_t len) {
    ...
    while (off < off_max) {
        ...
        T_D("Reading TLV of %u bytes @ 0x%08lx", tlvhdr.tlv_len, (long)off);
...
#if !defined(STANDALONE)
static void mount_ubifs_mfi(void) {
    int mtd_idx;
    int rc;
    ...
    mkdir("/mnt", 0700);
    info("Using device: %s\n", stage2_device);
    if (ERR(ubi_attach(mtd_idx), "Attach of data storage failed")) {
        goto MOUNT_TMP;
    }
    ...
    rc = ERR(mount("ubi0:switch", "/mnt", "ubifs", MS_SYNCHRONOUS | MS_SILENT, 0),
             "Mount of ubi0:switch failed");
    if (rc) {
        // print out a more helpful error message
        printf("Note: The preceding error message suggests that the UBIFS rootfs_data partition\n");
...
int main_mfi(CmdLine *c, uint64_t *t_ubifs) {
    ...
    *t_ubifs = uptime_milliseconds() - t_start;
    ...
    FATAL(mount("tmpfd", "/run", "ramfs", 0,
                "size=2097152,nosuid,nodev,mode=1777"),
          "Mount failed");
...
static void mount_ubifs_fit(void) {
    ...
    // Try to mount the UBIFS rootfs_data partition in the Flash
    rc = ERR(mount("ubi0:switch", "/switch", "ubifs", MS_SYNCHRONOUS | MS_SILENT, 0),
             "Mount of ubi0:switch failed");
    if (rc) {
        // print out a more helpful error message
        printf("Note: The preceding error message suggests that the UBIFS rootfs_data partition\n");
